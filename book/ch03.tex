...
\begin{table}[h!]
  \begin{center}
  \caption{一维非稳态热传导方程计算结果($\Delta t=1\mathrm{s}$)}
...
当改变时间步长$\Delta t=0.1\mathrm{s}$时，系数$\alpha\Delta t/(\Delta
x)^{2}=0.05$。重复上述过程，重新求解结果如表\ref{TbBD_1dht_result2}所示。
\begin{table}[h!]
  \begin{center}
    \caption{一维非稳态热传导方程计算结果($\Delta t=0.1\mathrm{s}$)}
...
若改变时间步长$\Delta t=2.5\mathrm{s}$时，系数$\alpha\Delta t/(\Delta
x)^{2}=1.25$。重复上述过程，重新求解结果如表\ref{TbBD_1dht_result3}所示
。
\begin{table}[h!]
  \begin{center}
    \caption{一维非稳态热传导方程计算结果($\Delta t=2.5\mathrm{s}$)}
