...
contains
    ...
    subroutine solver_maccormack_TVD(lambda)
        ...
        do i = di_1, n_mid
            do j = di_1, di_2
                associate (&
                    s => nint(speed_mid(i, j) / abs(spped_mid(i, j))))
                    if (abs(alfa_mid(i, j)) < eps) then
                        !r = zero
                        alfa_mid(i, j) = eps
                    end if
                    r = alfa_mid(i-s, j) / alfa_mid(i, j)
                end associate
                phi_mid(i, j) = r * (one + r) / (one + r**2)
            end do
        end do i
        ...
        do i = di_2, nl - di_1
            do j = di_1, di_2
                factor(i, j) = alfa_mid(i, j) * psi_mid(i, j) * &
                    (one - lambda * abs(speed_mid(i, j))) * &
                    (one - phi_mid(i, j))
            end do
            visco_d(i, 1) = factor(i, 1) + factor(i, 2)
            visco_d(i, 2) = factor(i, 1) * speed_mid(i, 1) + &
                factor(i, 2) * speed_mid(i, 2)
        end do
        ...
        do i = di_2, nl - di_1
            do j = di_1, di_2
                vec_u(i, j) = vec_u(i, j) + & 
                    lambda * (visco_d(i, j) - visco_d(i-1, j))
            end do
        end do
